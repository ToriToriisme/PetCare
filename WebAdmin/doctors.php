...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        $image_name = 'doctor-' . uniqid() . '.' . $ext;
        $upload_dir = '../assets/img/doctors/';
        if (!is_dir($upload_dir)) {
            mkdir($upload_dir, 0777, true);
        }
        move_uploaded_file($_FILES['image']['tmp_name'], $upload_dir . $image_name);
        $image_name = 'assets/img/doctors/' . $image_name;
    }

    if ($action === 'add') {
        $stmt = $conn->prepare("INSERT INTO doctors (name, degree, specialty, phone, email, description, image) VALUES (?,?,?,?,?,?,?)");
        $stmt->bind_param("sssssss", $name, $degree, $specialty, $phone, $email, $description, $image_name);
        $stmt->execute();
        $stmt->close();
    }
    ...
    if ($action === 'edit') {
        ...
        $stmt = $conn->prepare("UPDATE doctors SET name=?, degree=?, specialty=?, phone=?, email=?, description=?, image=? WHERE id=?");
        $stmt->bind_param("sssssssi", $name, $degree, $specialty, $phone, $email, $description, $image_name, $id);
        $stmt->execute();
        $stmt->close();
    }
    ...
    if ($action === 'delete') {
        $id = $_POST['id'];
        // Xóa file ảnh
        $stmt_get = $conn->prepare("SELECT image FROM doctors WHERE id=?");
        ...
        if($row && $row['image']) {
            $img_path = '../' . $row['image'];
            if(file_exists($img_path) && strpos($row['image'], 'assets/img/doctors/') !== false) {
                unlink($img_path);
            }
        }
                ...
                <tr>
                    ...
                    <td>
                        ...
                        if(!empty($d['image'])) {
                            ...
                            if(strpos($d['image'], 'assets/img/doctors/') !== false) {
                                $img_path = '../' . $d['image'];
                            } else {
                                $img_path = '../assets/img/doctors/' . basename($d['image']);
                            }
                        }
                        if($img_path && file_exists($img_path)): ?>
                            <img src="<?= $img_path ?>" width="60" height="60" style="object-fit:cover; border-radius:5px;" alt="<?= htmlspecialchars($d['name']) ?>">
                        <?php else: ?>
...
<div id="modal" style="display:none;position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);align-items:center;justify-content:center;">
    <div style="background:white;padding:20px;border-radius:8px;width:430px;">
        ...
        <form method="post" id="doctorForm" enctype="multipart/form-data">
            ...
            <input type="text" name="degree" id="degree" placeholder="VD: Thạc sĩ Thú Y (M.V.Sc)">
