      ...
      <?php foreach($doctors as $i=>$d): ?>
        <div class="expert-item <?= ($i%2)?'reverse':'' ?>">
          <div class="expert-image-box">
            <?php
              // Xử lý đường dẫn ảnh: nếu có full path thì lấy tên file, nếu không thì dùng trực tiếp
              $imgFile = 'default.jpg';
              if (!empty($d['image'])) {
                // Nếu có chứa đường dẫn, lấy tên file cuối cùng
                $imgFile = basename($d['image']);
              }
            ?>
            <img src="assets/img/doctors/<?= htmlspecialchars($imgFile) ?>" 
                 alt="<?= htmlspecialchars($d['name']) ?>" class="expert-img"
                 onerror="this.src='assets/img/doctors/default.jpg'">
          </div>
