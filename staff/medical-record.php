...
// Auto-create medical_records table if it doesn't exist
$table_check = $conn->query("SHOW TABLES LIKE 'medical_records'");
if ($table_check->num_rows == 0) {
    $create_table_sql = "CREATE TABLE IF NOT EXISTS medical_records (
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($stmt->execute()) {
        $stmt_update = $conn->prepare("UPDATE bookings SET status = 'completed' WHERE id = ?");
...
$record = null;
$stmt_rec = $conn->prepare("SELECT * FROM medical_records WHERE booking_id = ?");
$stmt_rec->bind_param("i", $booking_id);
$stmt_rec->execute();
